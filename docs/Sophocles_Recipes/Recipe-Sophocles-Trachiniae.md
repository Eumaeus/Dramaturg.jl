...
Using `scripts/config-trachiniae.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Sophocles/config-trachiniae.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Trachiniae/Trachiniae_beta_vocabulary.txt morph/source-data/words.txt
