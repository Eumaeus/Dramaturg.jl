# Recipe: Aeschylus' *Seven*

Using `scripts/config-aeschylus_seven.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aeschylus/config-seven.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus/Seven/Aeschylus_Seven_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-seven-analysis.txt 2> morph/output/aeschylus-seven-errors.log
