# Recipe: Euripides' *The Trojan Women*

Using `scripts/config-euripides_trojan.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_trojan.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Trojan/Euripides_Trojan_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-trojan-analysis.txt 2> morph/output/euripides-trojan-errors.log
