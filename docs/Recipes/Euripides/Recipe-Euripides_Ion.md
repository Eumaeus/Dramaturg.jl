# Recipe: Euripides' *Ion*

Using `scripts/config-euripides_ion.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_ion.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Ion/Euripides_Ion_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-ion-analysis.txt 2> morph/output/euripides-ion-errors.log
