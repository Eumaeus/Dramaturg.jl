# Recipe: Euripides' *Alcestis*

Using `scripts/config-euripides_alcestis.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_alcestis.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Alcestis/Euripides_Alcestis_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-alcestis-analysis.txt 2> morph/output/euripides-alcestis-errors.log
