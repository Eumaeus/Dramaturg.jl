# Recipe: Euripides' *Helen*

Using `scripts/config-euripides_helen.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_helen.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Helen/Euripides_Helen_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-helen-analysis.txt 2> morph/output/euripides-helen-errors.log
