# Recipe: Euripides' *Rhesus*

Using `scripts/config-euripides_rhesus.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_rhesus.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Rhesus/Euripides_Rhesus_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-rhesus-analysis.txt 2> morph/output/euripides-rhesus-errors.log
