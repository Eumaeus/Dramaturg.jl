# Recipe: Euripides' *Heracles*

Using `scripts/config-euripides_heracles.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_heracles.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Heracles/Euripides_Heracles_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-heracles-analysis.txt 2> morph/output/euripides-heracles-errors.log
