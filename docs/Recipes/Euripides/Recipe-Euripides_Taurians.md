# Recipe: Euripides' *Iphigenia among the Taurians*

Using `scripts/config-euripides_taurians.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_taurians.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Taurians/Euripides_Taurians_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-taurians-analysis.txt 2> morph/output/euripides-taurians-errors.log
