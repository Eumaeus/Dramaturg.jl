# Recipe: Euripides' *Orestes*

Using `scripts/config-euripides_orestes.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_orestes.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Orestes/Euripides_Orestes_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-orestes-analysis.txt 2> morph/output/euripides-orestes-errors.log
