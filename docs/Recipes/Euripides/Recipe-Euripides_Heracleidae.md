# Recipe: Euripides' *Heracleidae*

Using `scripts/config-euripides_heracleidae.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Euripides/config-euripides_heracleidae.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides/Heracleidae/Euripides_Heracleidae_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-heracleidae-analysis.txt 2> morph/output/euripides-heracleidae-errors.log
