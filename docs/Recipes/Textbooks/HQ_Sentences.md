# Recipe: Hansen & Quinn, Exercise Sentences 
Using `scripts/Configs/Textbooks/config-hq.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Textbooks/config-hq.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Textbooks/HQ_Sentences/HQ_Sentences_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/hq-analysis.txt 2> morph/output/hq-errors.log
