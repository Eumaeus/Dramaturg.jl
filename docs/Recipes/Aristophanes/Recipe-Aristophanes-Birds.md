# Recipe: Aristophanes' *Birds*

Using `scripts/Configs/Aristophanes/config-birds.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aristophanes/config-birds.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes/Birds/Aristophanes_Birds_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aristophanes-birds-analysis.txt 2> morph/output/aristophanes-birds-errors.log
