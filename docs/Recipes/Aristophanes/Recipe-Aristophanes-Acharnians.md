# Recipe: Aristophanes' *Acharnians*

Using `scripts/Configs/Aristophanes/config-acharnians.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aristophanes/config-acharnians.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes/Acharnians/Aristophanes_Acharnians_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aristophanes-acharnians-analysis.txt 2> morph/output/aristophanes-acharnians-errors.log
