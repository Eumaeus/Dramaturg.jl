# Recipe: Aristophanes' *Lysistrata*

Using `scripts/Configs/Aristophanes/config-lysistrata.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aristophanes/config-lysistrata.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes/Lysistrata/Aristophanes_Lysistrata_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aristophanes-lysistrata-analysis.txt 2> morph/output/aristophanes-lysistrata-errors.log
