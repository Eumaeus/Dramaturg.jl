# Recipe: Aristophanes' *Wealth*

Using `scripts/Configs/Aristophanes/config-wealth.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aristophanes/config-wealth.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes/Wealth/Aristophanes_Wealth_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aristophanes-wealth-analysis.txt 2> morph/output/aristophanes-wealth-errors.log
