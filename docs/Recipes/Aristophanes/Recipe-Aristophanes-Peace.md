# Recipe: Aristophanes' *Peace*

Using `scripts/Configs/Aristophanes/config-peace.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Aristophanes/config-peace.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes/Peace/Aristophanes_Peace_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aristophanes-peace-analysis.txt 2> morph/output/aristophanes-peace-errors.log
