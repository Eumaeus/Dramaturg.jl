# Recipe: Sophocles' *Trachiniae*

Using `scripts/Configs/Sophocles/config-sophocles-trachiniae.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Sophocles/config-sophocles-trachiniae.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Sophocles/Trachiniae/Trachiniae_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/trachiniae-analysis.txt 2> morph/output/trachiniae-errors.log
...
cp scripts/Configs/Sophocles/config-sophocles-philoctetes.toml scripts/config.toml
