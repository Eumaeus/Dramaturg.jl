# Recipe: Sophocles' *Antigone*

Using `scripts/Configs/Sophocles/config-antigone.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Sophocles/config-sophocles-antigone.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Sophocles/Antigone/Antigone_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/antigone-analysis.txt 2> morph/output/antigone-errors.log
