...
Using `scripts/config-odyssey.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Homer/config-odyssey.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Homer/Odyssey/Odyssey_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/odyssey-analysis.txt 2> morph/output/odyssey-errors.log
