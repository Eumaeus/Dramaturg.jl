...
Using `scripts/config-iliad.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs/Homer/config-iliad.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Homer/Iliad/Iliad_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/iliad-analysis.txt 2> morph/output/iliad-errors.log
