...
Using `scripts/config-hermes.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Homeric/config-hermes.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/The_Homeric_Hymn_to_Hermes/The_Homeric_Hymn_to_Hermes_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/hermes-analysis.txt 2> morph/output/hermes-errors.log
