...
Using `scripts/config-aphrodite.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Homeric/config-aphrodite.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/The_Homeric_Hymn_to_Aphrodite/The_Homeric_Hymn_to_Aphrodite_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aphrodite-analysis.txt 2> morph/output/aphrodite-errors.log
