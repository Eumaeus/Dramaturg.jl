...
Using `scripts/config-nt-mark.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_NT/config-nt-mark.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/NT_Mark/NT_Mark_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/nt-mark-analysis.txt 2> morph/output/nt-mark-errors.log
