# Recipe: Aeschylus' *Persians*

Using `scripts/config-aeschylus_persians.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aeschylus/config-aeschylus_persians.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus_Persians/Aeschylus_Persians_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-persians-analysis.txt 2> morph/output/aeschylus-persians-errors.log
