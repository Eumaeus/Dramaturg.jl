# Recipe: Aeschylus' *Suppliants*

Using `scripts/config-aeschylus_suppliants.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aeschylus/config-aeschylus_suppliants.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus_Suppliants/Aeschylus_Suppliants_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-suppliants-analysis.txt 2> morph/output/aeschylus-suppliants-errors.log
