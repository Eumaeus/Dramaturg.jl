# Recipe: Aeschylus' *Choephoroi*

Using `scripts/config-aeschylus_choephoroi.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aeschylus/config-aeschylus_choephoroi.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus_Choephoroi/Aeschylus_Choephoroi_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-choephoroi-analysis.txt 2> morph/output/aeschylus-choephoroi-errors.log
