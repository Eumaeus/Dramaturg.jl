# Recipe: Aeschylus' *Eumenides*

Using `scripts/config-aeschylus_eumenides.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aeschylus/config-aeschylus_eumenides.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus_Eumenides/Aeschylus_Eumenides_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-eumenides-analysis.txt 2> morph/output/aeschylus-eumenides-errors.log
