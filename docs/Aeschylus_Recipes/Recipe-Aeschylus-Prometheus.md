# Recipe: Aeschylus' *Prometheus*

Using `scripts/config-aeschylus_prometheus.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aeschylus/config-aeschylus_prometheus.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aeschylus_Prometheus/Aeschylus_Prometheus_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/aeschylus-prometheus-analysis.txt 2> morph/output/aeschylus-prometheus-errors.log
