...
Using `scripts/config-ecclesiazousae.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aristophanes/config-ecclesiazousae.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes_Ecclesiazousae/Aristophanes_Ecclesiazousae_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/ecclesiazousae-analysis.txt 2> morph/output/ecclesiazousae-errors.log
