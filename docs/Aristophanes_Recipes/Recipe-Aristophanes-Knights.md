...
Using `scripts/config-knights.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aristophanes/config-knights.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes_Knights/Aristophanes_Knights_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/knights-analysis.txt 2> morph/output/knights-errors.log
