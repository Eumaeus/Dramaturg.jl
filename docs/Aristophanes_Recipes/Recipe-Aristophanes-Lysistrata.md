...
Using `scripts/config-lysistrata.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Aristophanes/config-lysistrata.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Aristophanes_Lysistrata/Aristophanes_Lysistrata_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/lysistrata-analysis.txt 2> morph/output/lysistrata-errors.log
