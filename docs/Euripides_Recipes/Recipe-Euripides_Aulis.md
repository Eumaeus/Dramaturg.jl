# Recipe: Euripides' *Iphigenia at Aulis*

Using `scripts/config-euripides_aulis.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_aulis.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Aulis/Euripides_Aulis_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-aulis-analysis.txt 2> morph/output/euripides-aulis-errors.log
