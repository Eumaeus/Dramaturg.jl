# Recipe: Euripides' *Bacchae*

Using `scripts/config-euripides_bacchae.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_bacchae.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Bacchae/Euripides_Bacchae_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-bacchae-analysis.txt 2> morph/output/euripides-bacchae-errors.log
