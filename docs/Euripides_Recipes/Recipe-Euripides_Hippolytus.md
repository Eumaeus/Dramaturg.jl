# Recipe: Euripides' *Hippolytus*

Using `scripts/config-euripides_hippolytus.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_hippolytus.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Hippolytus/Euripides_Hippolytus_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-hippolytus-analysis.txt 2> morph/output/euripides-hippolytus-errors.log
