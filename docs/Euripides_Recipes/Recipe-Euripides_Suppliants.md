# Recipe: Euripides' *Suppliants*

Using `scripts/config-euripides_suppliants.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_suppliants.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Suppliants/Euripides_Suppliants_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-suppliants-analysis.txt 2> morph/output/euripides-suppliants-errors.log
