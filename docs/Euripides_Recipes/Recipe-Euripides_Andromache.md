# Recipe: Euripides' *Andromache*

Using `scripts/config-euripides_andromache.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_andromache.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Andromache/Euripides_Andromache_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-andromache-analysis.txt 2> morph/output/euripides-andromache-errors.log
