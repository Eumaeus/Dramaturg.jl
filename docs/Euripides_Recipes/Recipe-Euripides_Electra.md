# Recipe: Euripides' *Electra*

Using `scripts/config-euripides_electra.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_electra.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Electra/Euripides_Electra_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-electra-analysis.txt 2> morph/output/euripides-electra-errors.log
