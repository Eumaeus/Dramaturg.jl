# Recipe: Euripides' *Cyclops*

Using `scripts/config-euripides_cyclops.toml` as a starting point.

cd PATH/TO/Dramaturg/

[Start Docker]

cp scripts/Configs_Euripides/config-euripides_cyclops.toml scripts/config.toml

julia --project=. scripts/tokenize_cex.jl

cp data/vocabulary/Euripides_Cyclops/Euripides_Cyclops_beta_vocabulary.txt morph/source-data/words.txt
...
MORPHLIB=stemlib bin/cruncher -S < morph/source-data/words.txt > morph/output/euripides-cyclops-analysis.txt 2> morph/output/euripides-cyclops-errors.log
